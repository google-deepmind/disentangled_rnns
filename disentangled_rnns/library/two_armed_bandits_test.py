...
class TwoArmedBanditsTest(parameterized.TestCase):
  ...
  def test_softmax_stability(self, agent):
    ...
    self.assertAlmostEqual(np.sum(choice_probs), 1.0)
  ...
  def test_agent_network_rnn_types(self, agent_factory):
    """Smoke test that AgentNetwork works with both GRUs and DisRNNs."""
    agent = agent_factory()
    environment = two_armed_bandits.EnvironmentBanditsDrift(sigma=0.01, seed=0)
    n_steps = 5
    ...
    for _ in range(n_steps):
      ...
      self.assertAlmostEqual(np.sum(choice_probs), 1.0, places=5)
