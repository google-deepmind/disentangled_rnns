...
class DatasetRNNCategoricalWithActionMasks(DatasetRNNCategorical):
  ...
  def __init__(
    ...
    # output targets. The valid actions masks should never indicate that the
    # true action is not valid.
    timestep_indices, episode_indices = np.indices(
        (self._n_timesteps, self._n_episodes)
    )
    if not np.all(valid_actions_masks[timestep_indices, episode_indices, ys]):
      raise ValueError(
          'Valid actions masks must have True for the output targets.'
      )
