...
class DisRnnConfig:
  ...
  Attributes:
    ...
    choice_net_latent_penalty: Multiplier for KL cost on choice net (latent inputs) bottlenecks
    update_net_obs_penalty: Multiplier for KL cost on update net (observation inputs) bottlenecks
    update_net_latent_penalty: Multiplier for KL cost on update net (latent inputs) bottlenecks
...
class HkDisentangledRNN(hk.RNNCore):
  ...
  def __call__(self, observations: jnp.ndarray, prev_latents: jnp.ndarray):
    ...
    # Expand to have a separate copy per update network: (batch_size, obs_size, latent_size)
    ...
    # Expand to have a separate copy per update network: (batch_size, latent_size, latent_size)
...
def log_bottlenecks(params,
  ...
  update_latent_bottlenecks_closed = np.sum(update_latent_sigmas > closed_thresh)
